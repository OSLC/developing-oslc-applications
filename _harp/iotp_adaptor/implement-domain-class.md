...
The adaptor manager iotp-adaptorManager is the key class generated by the code generator that supports HTTP methods on the resource. All the CRUD methods for all the domain resources are generated as static methods in this class. User code needs to be added to implement these methods.

## Implement the Connector Manager create method

Implement the iotp-adaptorManager.create<resource)() method to create a new resource instance. This method uses the constructor to actually create the resource.
...
Implement the iotp-adaptorManager.query<resource>s() method to return all the instances of the resource. These methods get all the instances of a resource and are typical used to implement OSLC query as well as to provide information for selection dialogs.
...
Implement the iotp-adaptorManager.update<resource>() method to update a resource with a given id. This method has to json.remove any OSLC properties that are not settable through the IoT Platform REST APIs.
...
Implement the iotp-adaptorManager.delete{resource}() method to delete a resource with a given id.
...
Implement the iotp-adaptorManager.getETagFrom{resource}() method to identify a particular version of a resource. This is needed to ensure a resource hasn’t changed between a GETn and subsequent PUT.
...
Update IotPlatformService.createResourceAndChangeRequestAndRequirement to use the type parameter to determine which IoT Platform resource to create, and calls the appropriate iotp-adaptorManager.create method
